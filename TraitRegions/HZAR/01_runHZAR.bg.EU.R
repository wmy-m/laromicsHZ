...
colnames(meta) <- c("locality", "distance_meta","nSample")
data <- merge(
  data, 
  meta[, c("locality","nSample")], 
  by = "locality", 
  all.x = TRUE,   # keep all rows in data
  sort = FALSE    # preserve order
)
data$distance=NULL
colnames(data)[colnames(data) == "NA."] <- "distance"
### Load data and set parameters  -----------------------------------------
...
locus_id <- locus_columns[task_id]
...
save(fit_modelSelected, file = paste0("./EU.bg/EU.HZAR_v2_bgHI_task", task_id, ".RData"))
write.table(recheck_modelfit, 
            file = paste0("./EU.bg/EU.HZAR_v2_bgHI_convergence_failed.bestmodel_task", task_id, ".txt"),
