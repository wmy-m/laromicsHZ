...
#load dataset
load("NA.ChrW.Cline_input.RData")

# read in prior estimates of hybrid index
h<-read.table("NA.ChrW.Cline_input_h_est.txt",header=TRUE)
...
lb<-(k-1)*bsize + 1
#ub<-lb+bsize-1
ub <-5059 #for the last set (total snp number)
...
out<-paste("NA.ChrW.clinesOut",k,".rda",sep="")
