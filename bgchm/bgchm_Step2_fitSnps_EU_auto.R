...
#load dataset
load("EU.auto.Cline_input.RData")

# read in prior estimates of hybrid index
h<-read.table("EU.auto.Cline_input_h_est.txt",header=TRUE)
...
lb<-(k-1)*bsize + 1
#ub<-lb+bsize-1
ub <-134057 #for the last set (total snp number)
...
out<-paste("EU.auto.clinesOut",k,".rda",sep="")
