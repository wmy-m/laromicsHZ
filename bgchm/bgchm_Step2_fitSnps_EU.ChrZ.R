...
#load dataset
load("EU.ChrZ.Cline_input.RData")

# read in prior estimates of hybrid index
h<-read.table("EU.ChrZ.Cline_input_h_est.txt",header=TRUE)
...
lb<-(k-1)*bsize + 1
#ub<-lb+bsize-1
ub <- 3002 #for the last set (total snp number)
...
out<-paste("EU.ChrZ.clinesOut",k,".rda",sep="")
